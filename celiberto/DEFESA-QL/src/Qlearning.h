...
//*********** TABELA "Q" e "e" ************
// valores de estados aumentados para a tabela Q 
// para nao dar problema
...
void calcula_Qlambda(int estado, int estadoN, int acao, float reforcoAcao)
{
   ...
   Q[estado][acao]=
      (
        Q[estado][acao]+(alfa*delta*e[estado][acao])
      );
...
int Reforco(int gol,int ciclo,int acao,bool isBallCatchable,bool isBallKickable,int periodo,int jogo,float dist,float tempo)
{
  ...
  else if (isBallKickable && (acao==2 || acao==3)) {bomReforco=1;}
  else if(gol<golOld){bomReforco=-1;golOld=gol;}
...
int permitir_acao(int acao,float agenteX,float agenteY,float angulo)
{
	...
	if(acao==1)
	{
		if((agenteX>-32)&&(angulo>=-89 && angulo<=89))
		{
		  retorno_acao=6;	
		}
		else if((agenteX<-51)&&(angulo<=-91 || angulo>=91 ))
		{
		  retorno_acao=6;	
		}
...
void CarregarArquivo()
{
	...
	if(!tab_Q)
	{
		cout<< "Nenhuma tabela anterior encontrada. Gerando Tabelas !"<<endl;
...
void Salvaperiodo (int periodo, int gol_p, int jogo)
{
	...
	sprintf (file_name,"/home/sysop/Futebol/hashGoleiro-QL/src/Tabela/Periodo/gol_periodo.txt");
...
float ImprimeDiferencaQ(int periodo,float ValorQold)
{
	...
	FILE * arq;		
	sprintf (file_name,"/home/sysop/Futebol/hashGoleiro-QL/src/Tabela/Q/diferencaQ.txt");
    arq=fopen(file_name,"w");
	...
	ValorQnew[periodo]=fabs(ValorQold-somaQ);
...
void SalvaMAPA (float agenteX, float agenteY,bool gravar)
{
	if(gravar)
	{
	  ...
	  sprintf (file_name,"/home/sysop/Futebol/Goleiro-Ql/src/Tabela/Mapa/caminho.txt");
